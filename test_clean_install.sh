...
echo -e "${YELLOW}Step 3: Testing npm package installation (css-variable-lsp@1.0.10)...${NC}"
if command -v npm >/dev/null 2>&1; then
    # Create package.json to avoid npm installing in parent directory
    echo '{"name":"test","version":"1.0.0"}' > package.json
    npm install css-variable-lsp@1.0.10 --no-save 2>&1 | tail -5
...
echo -e "  ✓ npm package css-variable-lsp@1.0.10 is installable"
