...
- `--run-tests` runs `cargo test --locked --lib`, `./test_extension.sh`, and `./test_clean_install.sh`. The integration test verifies the tracked WASM byte-for-byte and fails on mismatch.
